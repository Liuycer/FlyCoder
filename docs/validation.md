...
- **修正后重跑**（共 19 题，全部 `deepseek-v4.1-flash`，`llm_model` 逐次记录）：`basic-001-002-prompted` 2/2（input 6,721 / output 1,092 tokens，002 无审查标记，早前多出的 `average()` 与参数改名确认为缺陷产物）；`basic-003-008-prompted` 6/6（12 次 HTTP 尝试、input 18,390 / output 4,039 tokens）；`intermediate-009-016-prompted` 8 题 7 次通过 + 016 在修复 JSON 解析后重跑通过；`advanced-017-020-prompted` 3/3 通过，018 入场被拒。以通过的那次运行计，每题 4 个动作、2 次 LLM 调用，无重试。
